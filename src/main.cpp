...
const string strMessageMagic = "Mjollnir Signed Message:\n";
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!IsCoinBase())
        return 0;
    return max(0, (COINBASE_MATURITY+8) - GetDepthInMainChain());
}
...
inline int64_t GetDevaluatedBlock( float rate, int nCycle )
{
	int i;
	float base = 50.0f;
	for( i = 0; i < nCycle; i ++ ) {
...
const int64 nInterval = 10; // 10 minutes for MNR
...
void ThreadScriptCheck() {
    RenameThread("mjollnircoin-scriptch");
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mjollnircoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Mjollnircoin: increase each by adding 1 to vertcoin's value.
//250,191,181,218
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MjollnircoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MjollnircoinMiner : ProcessBlock, block not accepted");
...
void static MjollnircoinMiner(CWallet *pwallet)
{
    printf("MjollnircoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mjollnircoin-miner");
    ...
    try { loop {
        ...
        printf("Running MjollnircoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MjollnircoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nGenerateThreads; i++)
        minerThreads->create_thread(boost::bind(&MjollnircoinMiner, pwallet));
}
