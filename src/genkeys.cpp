...
static int PRIVATE_KEY_MJOLLNIRCOIN_BYTE = 128;
static int PUBLIC_KEY_MJOLLNIRCOIN_BYTE= 50;
...
// WIF = PRIVATE_KEY_MJOLLNIRCOIN_BYTE + Hash(secret) + 4 byte checksum (take first 4 bytes from Hash(PRIVATE_KEY_MJOLLNIRCOIN_BYTE + Hash(secret)) )
std::vector<unsigned char> getPrivateKeyWIF(std::vector<unsigned char> secret)
{
    uint256 digest = Hash(secret.begin(), secret.end());

    std::vector<unsigned char> vch(1, PRIVATE_KEY_MJOLLNIRCOIN_BYTE);
...
// Get address
// Address = PUBLIC_KEY_MJOLLNIRCOIN_BYTE + Hash160(ECDSAPubKey) + 4 byte checksum
// ECDSAPubKey = EC_KEY_generate_pubkey(Hash(secret))
// checksum = first 4 bytes from Hash(PUBLIC_KEY_MJOLLNIRCOIN_BYTE + Hash160(ECDSAPubKey))
std::vector<unsigned char> getAddress(const std::vector<unsigned char> &vchSecret)
{
    ...
    std::vector<unsigned char> vch(1, PUBLIC_KEY_MJOLLNIRCOIN_BYTE);
       ...
       while (1)
         {
           ...
           switch (c)
             ...
             case 'p':
               ...
               break;
			 case 't':
			   PUBLIC_KEY_MJOLLNIRCOIN_BYTE = 112;
			   PRIVATE_KEY_MJOLLNIRCOIN_BYTE = 239;
...
int main(int argc, char **argv)
{
    ...
    std::string name("Mjollnircoin ");	
    if ( PRIVATE_KEY_MJOLLNIRCOIN_BYTE == 239 ) {
	name += "[testnet] ";
	}
